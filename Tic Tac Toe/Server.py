...
class TicTacToeServer:
            ...
            try:
                ...
                if self.is_valid_move(row, col, player_index):
                    ...
                    if self.check_winner(player_index):
                        self.send_message_to_all(f"Winner: {player_name}")
                        self.reset_board()
                    elif self.is_board_full():
                        self.send_message_to_all("It's a draw!")
                        self.reset_board()
                    else:
                        self.send_board_to_all()
                        self.switch_player()
                ...
                self.handle_player_disconnection(player_index)
                break
    ...
    def update_board(self, row, col, player_index):
        player_symbol = self.get_player_symbol(player_index)
